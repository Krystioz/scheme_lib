...
(define member? (lambda (a lat)(cond ((null? lat) #f)
...
(define insertL (lambda (new old l)
                  (cond ((null? l) (quote ()))
                        (else (cond ((eq? (car l) old) (cons new l))
...
(define expt (lambda (n m)
               (cond
                 ((zero? m) 1)
                 (else (x n (expt n (sub1 m)))))))

(define quotient (lambda (n m)
                   (cond
                     ((< n m) 0)
